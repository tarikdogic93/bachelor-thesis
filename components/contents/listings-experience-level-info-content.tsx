...
import ListingsExperienceLevelInfoChart from "@/components/charts/listings-experience-level-info-chart";

export default function ListingsExperienceLevelInfoContent() {
  const result = useQuery(api.analytics.getListingsExperienceLevelInfo);
      ...
      <Card className="w-80 sm:w-[400px] md:w-[500px]">
        <CardHeader className="border-b">
          <CardTitle className="break-words text-center">
            Analyzing experience levels by job sector
          </CardTitle>
        </CardHeader>
              ...
              {result.companiesWithJobListingsCount > 0 ? (
                <>
                  <div className="w-full flex-1">
                    <ListingsExperienceLevelInfoChart
                      data={result.listingsExperienceLevelInfo}
                    />
                  </div>
                </>
              ) : (
