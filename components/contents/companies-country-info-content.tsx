...
export default function CompaniesCountryInfoContent() {
        ...
        {result && result.usersWithCountryCount > 0 && (
          <CardFooter className="px-0">
            <div className="w-full text-center">
              <p className="font-semibold text-primary/80">{`Total companies: ${result.usersCount}`}</p>
              <p className="text-sm font-medium text-primary/50">{`Companies with country info provided: ${result.usersWithCountryCount}`}</p>
            </div>
          </CardFooter>
        )}
      </Card>
