...
import ListingsSettingTypeInfoChart from "@/components/charts/listings-setting-type-info-chart";

export default function ListingsSettingTypeInfoContent() {
  const result = useQuery(api.analytics.getListingsSettingTypeInfo);
      ...
      <Card className="w-80 sm:w-[400px] md:w-[500px]">
        <CardHeader className="border-b">
          <CardTitle className="break-words text-center">
            Visualize job listings across settings
          </CardTitle>
        </CardHeader>
            ...
            <>
              {result.companiesWithJobListingsCount > 0 ? (
                <div className="w-full flex-1">
                  <ListingsSettingTypeInfoChart
                    data={result.listingsSettingTypeInfo}
                  />
                </div>
              ) : (
