...
import ListingsEducationLevelInfoChart from "@/components/charts/listings-education-level-info-chart";

export default function ListingsEducationLevelInfoContent() {
  const result = useQuery(api.analytics.getListingsEducationLevelInfo);
      ...
      <Card className="w-80 sm:w-[400px] md:w-[500px]">
        <CardHeader className="border-b">
          <CardTitle className="break-words text-center">
            Exploring education levels in job sectors
          </CardTitle>
        </CardHeader>
              ...
              {result.companiesWithJobListingsCount > 0 ? (
                <>
                  <div className="w-full flex-1">
                    <ListingsEducationLevelInfoChart
                      data={result.listingsEducationLevelInfo}
                    />
                  </div>
                </>
              ) : (
